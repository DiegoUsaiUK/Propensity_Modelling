...
In any analytics study, there are several steps I would normally take to ensure an effective modelling strategy: 

- I would __fit several models__, compare their performance and pick the most suitable one for the “So what?” analysis

- I would implement __grid search__ to fine tune parameters for all models and improve their predictive performance

- In case of class imbalance, I would test __different resampling techniques__, compare their output and use the one that best suits the data structure and the analysis goals


In order to keep project running time reasonable, I made a number of choices. 

1 - First of all, I've opted for __h2o__ as my modelling platform as it offers a number of advantages: 
...
- I can enable __cross-validation__ to be carried out without the need for a separate `validation frame` to be "carved out" of the training set 

One aspect I __did not manage to cover__ in this project was __parameters fine tuning__ (a.k.a. grid search). This is mostly due to the limitations of my current laptop, which lacks adequate computing power: even enabling the built-in __h2o__ parallel processing had my machine freeze and crash several times. I will explore more computational hungry tasks when I'll eventually upgrade to a more powerful computer.  
...
It's now time to fit some models: for this project I'm estimating a __Generalised Linear Model__ (a.k.a. Elastic Net), a __Random Forest__ (which h2o refers to at _Distributed Random Forest_) and a __Gradient Boosting Machine__ (or GBM). Note that all of these models have 2 parameters in common: 
...
For the `tree-based` models (DRF and GBM) I’m also using some _control parameters_ to manage the models' estimation running time:
...
drf_model <- h2o.loadModel(path = "../03_models/DRF_model_R_1587120349395_19")

gbm_model <- h2o.loadModel(path = "../03_models/GBM_model_R_1587120349395_248")
```
...
There are many methodologies to interpret machine learning results (i.e. _local interpretable model-agnostic explanations_, _partial dependence plots_, _permutation-based variable importance_) but in this project I examine the `DALEX` package, which focuses on __Model-Agnostic Interpretability__ and provides a convenient way to comparing performance across multiple models with different structures. 

One of the key __advantages__ of the model-agnostic approach is that you can compare contributions of traditional "glass-box" models as well as black-box models on the same scale. However, being permutation-based, one of its main __drawbacks__ is that it do not deal well with large number of predictor variables and larger datasets.   
...
At last, I'm ready to pass the __explainer objects__ to several DALEX functions that will help assess and compare the performance of the different models.

## Assessing the models 

Given that performance measures may reflect a different aspect of the predictive performance of a model, it is important to evaluate and compare several metrics when appraising a model.

To evaluate and compare my models' performance, I've drawn inspiration from the framework used by Przemyslaw Biecek and Tomasz Burzykowski in their book, [__Explanatory Model Analysis__](https://pbiecek.github.io/ema/introduction.html), which is structured around key questions:
...
### __1 - Are the models well fitted?__
...
Based on the metrics available for all models ( __accuracy__ and __AUC__), I can see that __elastic net__ and __gradient boosting__ are performing roughly in par with one another, with __random forest__ not far behind. AUC ranges between .78-.80 whereas accuracy has a slightly narrower range of .89-.90
...
In spite of achieving the second highest AUC score, the random forest model appears to perform better when considering the median absolute residuals. Looking at the distribution of the residuals on the right-hand side, you can see that the median residuals are lowest for the random forest model. This is also mirrored by the boxplots on the left-hand side, where the random forest also has the lowest median absolute residual value.


### __2 - How do the models compare with one another?__

#### ROC and AUC

The __Receiver Operating Characteristic (ROC)__ curve is a graphical method that allows to visualise a classification model performance against a random guess, which is represented by the striped line on the graph. The curve plots the true positive rate (TPR) on the y-axis against the false positive rate (FPR) on the x-axis. 
...
plot(eva_glm, eva_dfr, eva_gbm, geom = "roc") +
  ggtitle("ROC Curves - All Models",  
          "AUC_glm = 0.750    AUC_drf = 0.785   AUC_gbm = 0.794") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```

The insight that can be gathered from a ROC curve is two-fold: 

* I can see that my model is performing much better than a random guess

* From this I can also calculate the [__AUC (Area Under the Curve)__](https://en.wikipedia.org/wiki/Receiver_operating_characteristic#Area_under_the_curve), which summarises the ROC curve and can be used to compare models performance - the perfect classifier would have AUC = 1. 

All models performs much better that random guessing and achieves a AUC of .75-.79, with the GBM achieving the highest score of 0.794.


### __3 - Which variables are important in the models?__
...
DALEX calculates variable importance measures via permutation, which is model agnostics, allowing for direct comparison between models of different structure. However, we should bear in mind that with variable importance scores based on permutations, calculations slow down when the number of features increases. 

Once again, I'm passing the "explainer" for each single model to the `feature_importance()` function and setting `n_sample` to 8000 to use practically all observations available. Although not exorbitant, the total execution time was __nearly 20 minute__ but this is based on a relatively small dataset. Always bear in mind that computation speed can be increased by reducing `n_sample`, which is especially important for larger datasets.
...
Now I only have to feed the __vip__ objects to a plotting function: as suggested by the auto-generated x-axis label ( __Drop-out loss__), the main intuition behind how variable importance is calculated lies in how much the model fit would decreases if the contribution of a selected explanatory variable was removed. The larger the segment, the larger the loss when that variable is dropped from the model.

```{r, cache=TRUE, eval=TRUE, include=T}
...
You can also notice the __distinct starting point__ for the x-axis left edge, which picks up on the difference in the RMSE loss between the three models: in this case the __elastic net__ model has the highest RMSE, suggesting the greater number of tail residuals seen earlier in the residual diagnostics is probably penalising the RMSE score.


### __4 - How does a single variable affect the average prediction?__

#### Partial Dependence profiles

After we have identified the relative predictive power of each variable, we may want to investigate how their relationship with the predicted response differ across all three models. __Partial Dependency (PD) plots__, sometimes also referred to as PD profiles, offer a great way to inspect how each model is responding to a particular predictor. 
...
Partial Dependency plots could also work as a diagnostic tool: looking at `poutcome` (outcome of the `previous` marketing campaign) reveals that __GBM__ and __random forest__ correctly picked up on a higher probability of signing up when the outcome of a previous campaign was `scs` (success). 
...
Based on my previous analysis experience of similar financial products, I would not advise a bank to ramp up their direct marketing activity around the weeks in the run to Christmas as this is a period of the year where consumers focus shifts away from this type of purchases.  
...
- Removes `previous`, which shows little difference between its 2 levels in the PD plot - it's also moderately correlated with `pdays`, suggesting that they may capturing the same behaviour
...
drf_final <- h2o.loadModel(path = "../03_models/DRF_model_R_1587207745751_1")
```
...
### A important remark on Partial Dependency Plots

Being already familiar with odds ratios in the context of a logistic regression, I set out to understand whether the same intuition could be extended to black-box classification models. 

During my research I've come across, among others, this very interesting post on [__Cross Validated__](https://stats.stackexchange.com/) that draws a parallel between [__odds ratio from decision tree and random forest__](https://stats.stackexchange.com/questions/93202/odds-ratio-from-decision-tree-and-random-forest).  

Basically, this means that partial dependency plots can be used in a similar way to how odds ratios to define what characteristics of a customer profile influence his/her propensity to performing a certain type of behaviour. 

For example, features like `job`, `month` and `contact` would provide context around __who__, __when__ and __how__ to target:

* Looking at `job` will tell us that a customer in an _admin_ role is 25% more likely to subscribe that a _self employed_.

* Contacting a prospective customer in the `month` of _October_ offers on average almost double the chance of a positive outcome than in _September_. 

* `contacting` your customer on their _mobile_ will nearly double the chances of subscription than a _telephone_ call.

- _NOTE THAT Partial Dependency Plots for all final model's predictors can be found in the __Appendix___

Armed with such insight, one can help __shaping overall communication plans__ by focusing marketing activities towards customers more likely to subscribe to a term deposit.
...
After estimating a number of models ( a _logistic regression_, a _random forest_ and a _gradient boosting machines_), I have used the __DALEX__ library to __assessed and compared their performance__ through an array of metrics. This library employs a __model-agnostic approach__ that enables to __compare traditional "glass-box" models and "black-box" models__ on the same scale. 
...
One of its key __advantages__ is the ability to compare contributions of traditional "glass-box" models as well as black-box models on the same scale. However, being permutation-based, one of its main __drawbacks__ is that it does not scale well to large number of predictor variables and larger datasets. 

Depending on the scale of your dataset and number of explanatory variables you want/need to include in your model, you should carefully consider whether to make DALEX your __ML Interpretability__ library of choice.
...
* For a in-depth look at tools and techniques to examine fully-trained machine-learning models as a whole, and compare their performance in a model-agnostic framework see: [__Explanatory Model Analysis__](https://pbiecek.github.io/ema/introduction.html), P. Biecek, T. Burzykowski

* For an advanced tutorial on sales forecasting and product backorders optimisation see: [__Predictive Sales Analytics: Use Machine Learning to Predict and Optimize Product Backorders__](https://www.business-science.io/business/2017/10/16/sales_backorder_prediction.html)
